...
export function getModifiedUserAgentStyle(
  ...
  lines.push(
    `    border-color: ${modifyBorderColor({ r: 76, g: 76, b: 76 }, filter)};`
  );
  lines.push(
  ...
  lines.push("a {");
  lines.push(
    `    color: ${modifyForegroundColor({ r: 0, g: 64, b: 255 }, filter)};`
  );
  lines.push("}");
  lines.push("table {");
  lines.push(
  ...
  lines.push(
    `    color: ${modifyForegroundColor({ r: 169, g: 169, b: 169 }, filter)};`
  );
  lines.push("}");
...
const awaitingForImageLoading = new Map<
  string,
  ((imageDetails: ImageDetails) => void)[]
>();
...
function getBgImageModifier(
  ...
  try {
        ...
        if (imageDetailsCache.has(url)) {
          ...
          try {
            ...
            if (awaitingForImageLoading.has(url)) {
              ...
              imageDetails = await new Promise<ImageDetails>((resolve) =>
    ...
    matches.forEach(({ match, type, index: matchStart }, i) => {
      ...
      modifiers.push(
        type === "url" ? getURLModifier(match) : getGradientModifier(match)
      );
